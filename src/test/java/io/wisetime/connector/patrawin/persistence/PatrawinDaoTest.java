...
class PatrawinDaoTest {
  ...
  void findClientsOrderedByCreationTime() {
    ...
    final Client createdNow1 = patrawinDaoTestUtils
        .createClient(fakeCaseClientGenerator.randomClient(now).number("123").build());
    final Client createdNow2 = patrawinDaoTestUtils
        .createClient(fakeCaseClientGenerator.randomClient(now).number("122").build());
    final Client createdYesterday = patrawinDaoTestUtils
        .createClient(fakeCaseClientGenerator.randomClient(now.minus(1, ChronoUnit.DAYS)).build());
    final Client createdLastWeek = patrawinDaoTestUtils
        .createClient(fakeCaseClientGenerator.randomClient(now.minus(7, ChronoUnit.DAYS)).build());
    final Client createdLongTimeAgo = patrawinDaoTestUtils
        .createClient(fakeCaseClientGenerator.randomClient(MIN_SQL_DATE_TIME).build());
